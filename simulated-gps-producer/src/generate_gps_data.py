...
def calculate_current_location_and_save(spark: SparkSession, minute: int) -> None:
    ...
    df_gps_data: DataFrame = (
        ...
        .select(
            ...
            "second",
            "microsecond",
            F.when(F.col("trip_ended"), F.col("fare_amount")).alias("fare_amount"),
            F.when(F.col("trip_ended"), F.col("tip_amount")).alias("tip_amount"),
            F.when(F.col("trip_ended"), F.col("total_profit")).alias("total_profit"),
            F.when(F.col("trip_ended"), F.col("PULocationID")).alias("pickup_location_id"),
            F.when(F.col("trip_ended"), F.col("PUBorough")).alias("pickup_borough"),
            F.when(F.col("trip_ended"), F.col("PUZone")).alias("pickup_zone"),
            F.when(F.col("trip_ended"), F.col("DOLocationID")).alias("dropoff_location_id"),
            F.when(F.col("trip_ended"), F.col("DOBorough")).alias("dropoff_borough"),
            F.when(F.col("trip_ended"), F.col("DOZone")).alias("dropoff_zone"),
        )
